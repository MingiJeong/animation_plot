...
def gps_sonde_callback(sonde_msg, gps_msg, airspeed_msg, local_velocity_msg):
    data_array = [gps_msg.header.stamp.to_sec(), cur_compass.header.stamp.to_sec(), float(cur_compass.range),
                airspeed_msg.header.stamp.to_sec(), float(airspeed_msg.groundspeed),
...
def spin(fig,ax):
    ...
    ts = message_filters.ApproximateTimeSynchronizer([sonde_sub, gps_sub, airspeed_sub, local_velocity_sub], 100, 0.5, allow_headerless=True)
